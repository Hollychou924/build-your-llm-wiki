...
- 用户后续很可能会发来**同一篇文章的不同来源**：比如先发二手解读，再发官方一手稿。
- 这时不要机械重复建档。
- 正确做法是：
  - 先查知识库是否已有同源条目
  - 若已有，则改做“同源复核”
  - 补评级、补新增事实、记 daily/memory，但不要长重复肉

## 文章评级

- 评级不要写进标题，标题保持干净可读。
- 评级更适合写在 raw 文件顶部的 `## 文章评级`。
- 不要只打 tag；tag 只能筛选，表达不了“新知识多”还是“表达框架强”。
- 最少也要区分：`知识增量` vs `表达价值`，否则会低估那些“事实不新但解释极强”的文章。
